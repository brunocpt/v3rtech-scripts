...
# Versão: 4.0.5
# Data: 2026-02-24
# Objetivo: Instalação e restauração de ferramentas de desenvolvimento via Flatpak
...
if [ -x /usr/bin/code ]; then
  ...
  if ! flatpak list | grep -q com.visualstudio.code; then
    ...
    if [ -f "/mnt/trabalho/Cloud/Compartilhado/Linux/config/vscode-flatpak-$USER.zip" ]; then
      cp /mnt/trabalho/Cloud/Compartilhado/Linux/config/vscode-flatpak-$USER.zip $HOME
      cd $HOME
...
# --- PyCharm ---
  ...
  #if [ -f "/mnt/trabalho/Cloud/Compartilhado/Linux/config/pycharm-flatpak-$USER.zip" ]; then
    #cp /mnt/trabalho/Cloud/Compartilhado/Linux/config/pycharm-flatpak-$USER.zip $HOME
    #cd $HOME
...
if ! flatpak list | grep -q io.dbeaver.DBeaverCommunity; then
  ...
  if [ -f "/mnt/trabalho/Cloud/Compartilhado/Linux/config/dbeaver-flatpak-$USER.zip" ]; then
    cp /mnt/trabalho/Cloud/Compartilhado/Linux/config/dbeaver-flatpak-$USER.zip $HOME
    cd $HOME
...
if ! flatpak list | grep -q com.getpostman.Postman; then
  ...
  if [ -f "/mnt/trabalho/Cloud/Compartilhado/Linux/config/postman-flatpak-$USER.zip" ]; then
    cp /mnt/trabalho/Cloud/Compartilhado/Linux/config/postman-flatpak-$USER.zip $HOME
    cd $HOME
...
if [ -x /usr/bin/remmina ]; then
  ...
  if ! flatpak list | grep -q org.remmina.Remmina; then
    ...
    if [ -f "/mnt/trabalho/Cloud/Compartilhado/Linux/config/remmina-flatpak-$USER.zip" ]; then
      cp /mnt/trabalho/Cloud/Compartilhado/Linux/config/remmina-flatpak-$USER.zip $HOME
      cd $HOME
