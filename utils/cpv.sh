...
# Versão: 4.1.0
# Data: 2026-02-25
# Objetivo: Organizar e mover arquivos de vídeo usando FileBot e rsync.
...
(
  ...
  rsync -rltD --info=name1 --update --remove-source-files --exclude-from=/mnt/trabalho/Cloud/Compartilhado/Linux/v3rtech-scripts/configs/exclude-list.txt "$src_dir" "$dest_dir" | while IFS= read -r line; do
      [ -n "$line" ] && echo "#$line"
  done
