#!/bin/bash
# ==============================================================================
# Script: lib/install-apps-games.sh
# Versão: 4.7.0
# Data: 2026-02-25
# Objetivo: Instalar aplicativos de Jogos
...
section "Instalação de Aplicativos de Jogos"

# Carrega seleção de apps
if [ -f "$CONFIG_HOME/selected-apps.conf" ]; then
    source "$CONFIG_HOME/selected-apps.conf"
else
    log "WARN" "Arquivo de seleção não encontrado. Nenhum app de Jogos será instalado."
    exit 0
fi

log "STEP" "Instalando aplicativos de Jogos selecionados..."
...
if [ $installed_count -eq 0 ]; then
    log "INFO" "Nenhum aplicativo de Jogos foi selecionado para instalação."
else
    log "SUCCESS" "Instalação de $installed_count aplicativo(s) de Jogos concluída!"
fi
