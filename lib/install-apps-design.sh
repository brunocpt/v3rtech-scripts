#!/bin/bash
# ==============================================================================
# Script: lib/install-apps-design.sh
# Versão: 4.0.5
# Data: 2026-02-24
# Objetivo: Instalar aplicativos de Design e Imagem
...
section "Instalação de Aplicativos de Design"

# Carrega seleção de apps
if [ -f "$CONFIG_HOME/selected-apps.conf" ]; then
    source "$CONFIG_HOME/selected-apps.conf"
else
    log "WARN" "Arquivo de seleção não encontrado. Nenhum app de Design será instalado."
    exit 0
fi

log "STEP" "Instalando aplicativos de Design selecionados..."
...
if [ $installed_count -eq 0 ]; then
    log "INFO" "Nenhum aplicativo de Design foi selecionado para instalação."
else
    log "SUCCESS" "Instalação de $installed_count aplicativo(s) de Design concluída!"
fi
